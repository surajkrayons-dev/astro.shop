...
class StoreUserMappingImport implements ToModel, WithHeadingRow, WithValidation
{
    ...
    public function model(array $row)
    {
        try {
            ...
            // Uniqueness check for current import batch
            ...
            // Check if any other promoter already mapped to this store for this client/month/year
            $duplicateMapping = StoreUserMapping::where([
                'store_id'  => $store->id,
                'client_id' => $client->id,
                'month'     => $month,
                'year'      => $year,
            ...
            if ($duplicateMapping) {
                throw new \Exception("Row #{$rowNumber}: A mapping for this store in the given month and year already exists.");
            }
            ...
            // Check if record already exists for same promoter
