...
class SubAdminImport implements ToModel, WithHeadingRow, WithValidation
{
    ...
    public function model(array $row)
    {
        try {
            ...
            $code = trim($row['sub_admin_code'] ?? '');
            $clientCodeRaw = trim($row['client_code'] ?? '');
            $uniqueKey = strtolower($code);

            if (empty($code)) {
                throw new \Exception("Row #{$rowNumber}: 'Sub Admin Code' is required.");
            }
            ...
            if (isset($this->seen[$uniqueKey])) {
                ...
                throw new \Exception("Row #{$rowNumber}: Duplicate Sub Admin Code '{$code}' also found on row #{$previousRow}.");
            }
            ...
            if ($row['city'] && !$city) throw new \Exception("Row #{$rowNumber}: City '{$row['city']}' not found.");

            $state = $row['state'] ? State::where('name', $row['state'])->first() : null;
            if ($row['state'] && !$state) throw new \Exception("Row #{$rowNumber}: State '{$row['state']}' not found.");

            $country = $row['country'] ? Country::where('name', $row['country'])->first() : null;
            if ($row['country'] && !$country) throw new \Exception("Row #{$rowNumber}: Country '{$row['country']}' not found.");
            ...
            if (!empty($row['region_code'])) {
                ...
                // If not found, just keep $region as null (no exception thrown)
            }

            $dateOfJoining = $this->parseDate($row['date_of_joining'] ?? null);
            $dob = $this->parseDate($row['dob'] ?? null);
            $roleId = $this->getRoleId($row['role']);
            if (!$roleId) throw new \Exception("Invalid role '{$row['role']}' provided on row #{$rowNumber}.");

            $attributes = [
                'username'        => $username,
                'email'           => $email,
                'code'            => $code,
                'name'            => $row['sub_admin_name'],
                'mobile'          => $mobile,
                'dob'             => $dob,
            ...
            // ✅ Set client_id as comma-separated string
            if (!empty($clientIds)) {
            ...
            $msg = $th->getMessage();
            if (str_contains($msg, 'Integrity constraint violation') && str_contains($msg, 'Duplicate entry')) {
