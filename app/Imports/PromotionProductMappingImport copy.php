...
class PromotionProductMappingImport implements ToModel, WithHeadingRow, WithValidation
{
    ...
    public function model(array $row)
    {
        try {
            ...
            $uniqueKey = strtolower("{$promotionProductCode}|{$chainCode}|{$month}|{$year}");

            if (isset($this->seen[$uniqueKey])) {
                $previousRow = $this->seen[$uniqueKey];
                throw new \Exception(
                    "Row #{$rowNumber}: Duplicate (Promotion Product Code: '{$promotionProductCode}', Chain Code: '{$chainCode}', Month: '{$month}', Year: '{$year}') also found on row #{$previousRow}."
                );
            }
