...
class StoreUserMappingImport implements ToModel, WithHeadingRow, WithValidation
{
    ...
    public function model(array $row)
    {
        try {
            ...
            $uniqueKey = strtolower($storeCode . '|' . $userCode . '|' . $clientCode);
            if (isset($this->seen[$uniqueKey])) {
                $previousRow = $this->seen[$uniqueKey];
                throw new \Exception(
                    "Row #{$rowNumber}: Duplicate combination (Store: '{$storeCode}', User: '{$userCode}', Client: '{$clientCode}') also found on row #{$previousRow}."
                );
            }
