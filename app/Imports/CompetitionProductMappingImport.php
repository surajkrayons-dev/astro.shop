...
class CompetitionProductMappingImport implements ToModel, WithHeadingRow, WithValidation
{
    ...
    public function model(array $row)
    {
        try {
            ...
            $uniqueKey = strtolower("{$chainCode}|{$competitionProductCode}|{$regionCode}|{$formatCode}|{$month}|{$year}");

            if (isset($this->seen[$uniqueKey])) {
                $previousRow = $this->seen[$uniqueKey];
                throw new \Exception(
                    "Row #{$rowNumber}: Duplicate (Chain Code: '{$chainCode}', Competition Product Code: '{$competitionProductCode}', Region Code: '{$regionCode}', Format Code: '{$formatCode}', Month: '{$month}', Year: '{$year}') also found on row #{$previousRow}."
                );
            }
