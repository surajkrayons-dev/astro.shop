...
class FormatImport implements ToModel, WithHeadingRow, WithValidation
{
    /**
    ...
    public function model(array $row)
    {
        try {
            ...
            if (isset($this->seen[$uniqueKey])) {
                ...
                throw new \Exception(
                    "Row #{$rowNumber}: Duplicate (Format Code: '{$code}') also found on row #{$previousRow}."
                );
            }
            ...
            if ($existing) {
                ...
                $attributes['code']       = $row['format_code'];
                return \App\Models\Format::create($attributes);
            }
    ...
    {
        return [
            '*.format_code' => 'required|max:200',
            '*.format_name' => 'required|max:200',
            '*.status' => 'required',
        ];
    }
}
