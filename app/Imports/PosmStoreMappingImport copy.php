...
class PosmStoreMappingImport implements ToModel, WithHeadingRow, WithValidation
{
    ...
    public function model(array $row)
    {
        try {
            ...
            $uniqueKey = strtolower("{$posmCode}|{$storeCode}|{$month}|{$year}");

            if (isset($this->seen[$uniqueKey])) {
                $previousRow = $this->seen[$uniqueKey];
                throw new \Exception(
                    "Row #{$rowNumber}: Duplicate (Posm: '{$posmCode}', Store: '{$storeCode}', Month: {$month}, Year: {$year}) also found on row #{$previousRow}."
                );
            }
