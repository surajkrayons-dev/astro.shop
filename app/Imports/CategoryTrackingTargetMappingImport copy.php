...
class CategoryTrackingTargetMappingImport implements ToModel, WithHeadingRow, WithValidation
{
    ...
    public function model(array $row)
    {
        try {
            ...
            $uniqueKey = strtolower($categoryTrackingCode . '|' . $storeCode . '|' . $regioncode . '|' . $formatcode);
            if (isset($this->seen[$uniqueKey])) {
                ...
                throw new \Exception(
                    ...
                    . "Store Code: '{$storeCode}', Region Code: '{$regioncode}', Format Code: '{$formatcode}') also found on row #{$previousRow}."
                );
            }
